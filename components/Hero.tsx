    ...
    <section ref={containerRef} className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-brand-orange dark:bg-brand-dark transition-colors duration-500 pt-20">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10 flex flex-col md:flex-row items-center md:items-end justify-between h-full pb-20 md:pb-32">
        ...
        <div ref={textRef} className="text-white max-w-3xl">
          <h3 className="hero-text-reveal text-sm md:text-base font-bold tracking-[0.2em] mb-4 uppercase opacity-80 dark:text-brand-accent">Full-Stack Engineer</h3>
          <h1 className="hero-text-reveal font-display font-bold text-[4.5rem] md:text-[7rem] lg:text-[8.5rem] leading-[0.85] uppercase mb-6">
            Building<br/>Scalable<br/>Systems
          </h1>
          <div className="hero-text-reveal flex flex-col md:flex-row md:items-center gap-6 mt-8">
            <p className="md:max-w-md text-sm md:text-base opacity-90 leading-relaxed font-sans border-l-2 border-white/30 pl-4">
              I specialize in building robust, high-performance web applications using modern technologies like React, Node.js, and Cloud Infrastructure.
            </p>
            <div className="w-12 h-12 rounded-full border-2 border-white flex items-center justify-center animate-spin-slow">
              <div className="w-2 h-2 bg-white rounded-full"></div>
            </div>
          </div>
          <div className="hero-text-reveal mt-10">
            <Button icon onClick={scrollToAbout}>View My Work</Button>
          </div>
        </div>
